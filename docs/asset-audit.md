...
The inherited dependency audit reports **two moderate advisories**. They were not rechecked through a registry in this session, and their IDs/dependency paths are not supplied by the inherited summary. Do not guess those details or state the audit is clean. The parent must run `pnpm evidence audit`, which preserves the high-threshold command and an additional all-severity (`--audit-level=info`) detailed JSON report, including each advisory/version/path/fix. Every moderate finding receives an explicit unresolved disposition until reviewed in `docs/audit-dispositions.json` at the PR head; see the [audit procedure](release-readiness.md#audit-evidence-and-moderate-dispositions). A zero exit at the high threshold can coexist with both moderate findings; it is not zero vulnerabilities.
