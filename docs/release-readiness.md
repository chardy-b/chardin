...
## WIL-125 implementation handoff (2026-09-08)

The uncommitted implementation based on `92461e42f15b89672decc0d508e9d2bc2767f4d9` adds the original seamless pavilion, collision/support camera, visitor-started score and HTML alternative. [Its A01–A14 ledger](design/wil125-validation.md) separates passing controller evidence from pending exact committed-head CI and independent review. Fresh nonbrowser validation and integrity/hygiene results are recorded in `.hermes/execution/chardin/wil125-review-prep-result.md`.

Controller evidence passed: focused skyspace visuals 2/2 desktop/mobile with 14 genuine captures per viewport; full Playwright 29 passed, five intentional project skips, zero failures in 5m38s; eight separately isolated approach/threshold/chamber/exit × desktop-high/mobile-emulation-low performance collections with fixed 60 warmup + 300 raw samples. All ran with zero retries. Five exact desktop/mobile spawn/traveled and tablet replacement baselines passed behavioral tests and controller visual inspection; current hashes are inventoried and zero-diff assertions remain. The capture helper uses standard page-level viewport screenshots (`fullPage: false`), animations allowed, CSS scale, a 30s screenshot timeout, deterministic frame finish plus one presentation-only RAF, and a 480s visual fixture. The reduced-motion test synchronizes browser/canvas/engine within 15s after the controller proved event delivery lags `emulateMedia`; production semantics remain intact and the temporary console diagnostic is removed. The controller dependency audit high reported no known vulnerabilities.

These live-tree results do not establish exact committed-head CI, full-history secret scanning, CodeQL or independent spec/security/quality/originality approval. Remaining acceptance cases require retained evidence; physical-device/thermal/Safari/iOS, numeric budget owner decisions and strict runs, Wave 1 disposition, original/inherited asset rights and repository licensing remain open. This preparation runs no browser, Playwright, Next server, publication or deployment operation. Passing collections and accepted local baselines do not grant release or public redistribution permission.
