...
export class ChardinOutlineEffect extends Effect {
  constructor(normals: Texture) {
    super("ChardinOutline", outlineFragment, {
      ...
      uniforms: new Map<string, Uniform>([
        ...
        ["depthStrength", new Uniform(0.62)],
        ["normalStrength", new Uniform(0.28)],
      ]),
    })
  }
  ...
  setSignals(depth: boolean, normals: boolean) {
    this.uniforms.get("depthStrength")!.value = depth ? 0.62 : 0
    this.uniforms.get("normalStrength")!.value = normals ? 0.28 : 0
  }
  dispose() {
